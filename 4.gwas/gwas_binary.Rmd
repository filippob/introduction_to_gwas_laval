...
```{r data, echo=TRUE}
setwd("~/Dropbox/cursos/laval2019/introduction_to_gwas/")

## Prepare data
pathD <- paste("../imputation/dogs_imputed", c(".bed", ".bim", ".fam"), sep = "")
...
N. **`r nrow(map)` SNPs** from **`r nrow(genotypes$genotypes)` dog samples**. SNPs are distributed over **`r length(unique(map$chr))` chromosomes** (chr: `r unique(map$chr)`); dog samples belong to **`r length(unique(genotypes$fam$pedigree))` population** (populations: `r unique(genotypes$fam$pedigree)`)
...
names(phenotypes) <- c("id", "population", "condition", "sex")
```
...
save(genData, file = "steps/genData_binary.RData")

### analysis
load("steps/genData_binary.RData")
```
